...
else	// not POST
{
	...
	<head>
		...
		<title>Ruby deMure - Manage Collections Sort Order</title>
	...
	<td align="center">
		...
		$qCollection = "select
                    CollectionID, Title, Date, Priority, Filepath AS DefaultImage
                from Collection
                left outer join ProductImage on ProdImageID = (select MIN(DefaultImageID) from Product where Product.CollectionID = Collection.CollectionID)
                where Current = 1 /* current collections only */
		...
		<div class="column" id="sortList">
			...
			foreach ($collections as $collection) {	
				// The default image
				$Filepath = $collection['DefaultImage'];
				...
				<div class="portlet" id="<?php echo $collection['ID']; ?>">
					<div class="portlet-header"><?php echo strtoupper($collection['Title']) ?></div>
					<div class="portlet-content">
						<img src="<? echo $FilepathThumb; ?>" alt="IMAGE NOT FOUND" border="0" style="float:left;" />
						<?php
						echo 'ID: '.$collection['ID'];
						echo '<br/>Date: '.$collection['Date'];
						echo '<br/>Current Priority: '.$collection['Priority'];
						?>
					</div>
				</div>
				<?php
